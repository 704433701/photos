...
- (void)viewDidLoad
{
    [super viewDidLoad];
    self.tableView = ({
        UITableView *tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, (self.view.frame.size.height - 54 * 7) / 2.0f, WIDTH * 2 / 3, 54 * 8) style:UITableViewStylePlain];
...
{
    NSArray *titles = @[@"首页", @"普通密码", @"图形密码", @"指纹密码", @"伪装计算器", @"设置普通密码", @"设置图形密码", @"更多"];
...
- (void)switchAction:(UISwitch *)switc
{
    ...
    if (index == 1) {
        ...
        UISwitch *swit2 = (UISwitch *) [self.view viewWithTag:204];
        [_user setBool:NO forKey:@"4"];
        [swit2 setOn:NO animated:YES];
    }
    if (index == 2) {
    ...
    if (index == 4) {
        UISwitch *swit1 = (UISwitch *) [self.view viewWithTag:201];
        [_user setBool:NO forKey:@"1"];
        [swit1 setOn:NO animated:YES];
        UISwitch *swit2 = (UISwitch *) [self.view viewWithTag:202];
        [_user setBool:NO forKey:@"2"];
        [swit2 setOn:NO animated:YES];
        if (switc.on && ![_user boolForKey:@"firstCalulator"]) {
            [self firstCalulator];
        }
    }
...
- (void)firstCalulator
{
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"友情提示" message:@"解锁伪装计算器的密码为普通密码,输入完密码之后点击两次\".\"便可解锁,请确认您已记住解锁方式" delegate:self cancelButtonTitle:@"不在提醒" otherButtonTitles:@"确认", nil];
    alert.tag = 10020;
    [alert show];
...
{
    if (alertView.tag == 10020 && buttonIndex == 0) {
        [[NSUserDefaults standardUserDefaults] setBool:YES forKey:@"firstCalulator"];
    }
}
