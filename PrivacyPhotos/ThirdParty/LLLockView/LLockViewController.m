...
@interface LLockViewController ()<RESideMenuDelegate>
{
    int nRetryTimesRemain; // 剩余几次输入机会
}

// 上一界面截图
@property (nonatomic, retain) UIImageView *preSnapImageView;
@property (nonatomic, retain) UIImageView *currentSnapImageView; // 当前界面截图
@property (nonatomic, retain) LLLockIndicator* indecator;// 九点指示图
@property (nonatomic, retain) LLLockView* lockview;// 触摸田字控件
@property (nonatomic, retain) UILabel *tipLable;
@property (nonatomic, retain) UIButton *tipButton; // 重设/(取消)的提示按钮
...
- (void)viewDidLoad
{
    ...
    self.lockview = [[LLLockView alloc] initWithFrame:CGRectMake(0, (HEIGHT - WIDTH) / 2 + 32, WIDTH, WIDTH)];
    [self.view addSubview:_lockview];
    self.indecator = [[LLLockIndicator alloc] initWithFrame:CGRectMake((WIDTH - 34) / 2, (HEIGHT - WIDTH - 34) / 2 + 32, 34, 34)];
    [self.view addSubview:_indecator];
    
    self.tipLable = [[UILabel alloc] initWithFrame:CGRectMake(0, (HEIGHT - WIDTH - 34 - 40) / 4 + 40, WIDTH, 40)];
    [self.view addSubview:_tipLable];
    _tipLable.textAlignment = NSTextAlignmentCenter;
    
    self.tipButton = [UIButton buttonWithType:UIButtonTypeSystem];
    _tipButton.frame = CGRectMake(0, HEIGHT - (HEIGHT - WIDTH) / 4 - 20, WIDTH, 40);
    [self.view addSubview:_tipButton];
    [_tipButton addTarget:self action:@selector(tipButtonPressed:) forControlEvents:UIControlEventTouchUpInside];
    ...
    // 验证密码错误
        ...
        if (nRetryTimesRemain > 0) {
            ...
            if (1 == nRetryTimesRemain) {
                ...
                [self setErrorTip:[NSString stringWithFormat:@"密码错误，请重新输入"]
                        errorPswd:string];
            }
